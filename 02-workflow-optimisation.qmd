...
$\text{pNA} = \frac{(n - 5)}{n} = 0.45$, so we keep peptides that
are observed in at least 55% of the samples. This is an arbitrary
...
qf <- sweep( #4. Subtract log2 norm factor matrix diann (MARGIN = 1:2)
  qf,
  MARGIN = 1:2,
  STATS = get_diann_nf(qf, i="precursors_log"), #3.
  i = "precursors_log",
