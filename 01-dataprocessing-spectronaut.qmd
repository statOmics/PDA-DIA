...
```{r}
precursors <- precursors |> 
  mutate(species = grepl(pattern = "ups",PG.ProteinGroups) |> 
           as.factor() |>
...
```{r}
qf <-  qf |>
  filterFeatures(~ EG.Qvalue <= 0.01 & #1.
                     PG.Qvalue <= 0.01 &  #1.
#                     Lib.Q.Value <= 0.01 & #1.
#                     Lib.PG.Q.Value <= 0.01 & #1.
                     EG.PrecursorId != "" & #2.
                     EG.IsDecoy == 0 & #3. (Note, that this filter is not available with previous versions of DIA-NN, as the report.tsv file did not include a Decoy column. So other strategies are needed if Decoys are in the output file.)
                     PEP.IsProteotypic == 1) #4.
```
...
pepsPerProtDf <- qf[["precursors"]] |> 
    rowData() |> 
    data.frame() |>
    dplyr::select("PEP.StrippedSequence", "PG.ProteinAccessions") |>
    group_by(PG.ProteinAccessions) |>
    mutate(pepsPerProt = PEP.StrippedSequence |>
               unique() |> length()
           ) #1.
