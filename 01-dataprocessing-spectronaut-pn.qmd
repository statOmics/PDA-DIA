---
title: Differential abundance analysis with Spectronaut - starting from FG_MS2Quantity or FG_Quantity
code-tools: true
author:
  - name: Lieven Clement
...
The main search output for this Spectronaut version was stored in the report.parquet file , which can be found under data/spikein248-staesetal2024-spectronaut.parquet
...
- raw MS1 auantity: FG_MS1RawQuantity, normalised MS1 quantity: FG_MS1Quantity, MS2 Precursor quantities: FG_MS2RawQuantity, Normalised MS2 Precursor quantities: FG_MS2Quantity, etc., which are all at the precursor level 
- MS2 based summary at the protein (protein group)-level: PG_MaxLFQ

Here, we will use the `FG_MS2Quantity` column.
...
```{r import_data}
precursorFile <- "https://github.com/statOmics/PDA-DIA/raw/refs/heads/main/data/spikein248-staesetal2024-spectronaut.parquet" 
```
...
Note, that this is often also stored as FG_Quantity. 
Depending on the choice of the user either FG_MS1Quantity or FG_MS2Quantity are stored as FG_Quantity. 
These are already interference corrected and normalised precursor quantities.
...
No normalisation is required, as it already has been done internally by Spectronaut if you use precursors$FG_MS2Quantity. 

Note, that Spectronaut also provides the normalisation factors on the original scale. 
They are stored in the column precursors$EG_NormalizationFactor. 
...
```{r}
precursors |> 
  ggplot(aes(x=EG_NormalizationFactor * FG_MS2RawQuantity, y = FG_MS2Quantity)) +
  geom_point() + 
  theme_minimal()
```

Note, that the normalisation factor is different for each entry. 
Indeed, Spectronaut first conducts a kind of global normalisation by calculating a sample level normalisation factor. 
