# Tutorial: Precursor.Normalised + maxLFQ {-}
...
Note, that the retention time normalisation assumes that all samples have the same complexity across retention time. This typically works well for many experiments. However, this does not apply for e.g. single cell data, where the complexity can vary drastically from cell to cell. 
...
Another common approach for data exploration is to
...
```{r}
qf[targetProtein, , c("precursors_log", "proteins")] |> #1
  longForm(colvars = colnames(colData(qf))) |> #2
  data.frame() |> 
  ggplot() +
  aes(x = colname,
