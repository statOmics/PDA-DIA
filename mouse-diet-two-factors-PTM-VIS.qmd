...
```{r}
library(omicsGMF)
seP <- getWithColData(qf, "precursorsPTM_norm") |> 
  as("SingleCellExperiment")
X <- model.matrix(~diet*sex,colData(se))
seP <- runGMF(
    seP, 
    X = X,
    family = gaussian(), 
...
```{r}
plotReducedDim(seP, dimred = "GMF", colour_by = "sampleGroup")
```
...
```{r}
plotReducedDim(seN, dimred = "GMF", colour_by = "sampleGroup")
```